...
def NFBlock(in_channel, out_channel, kernel_size = 3, alpha = 0.2, beta = 1.0, stride = 1, group_size = 128, big_width = True, expansion = 0.5, use_two_convs = True, se_ratio = 0.5, stochdepth_rate = None):
  ...
  results = WSConv2D(group_size * (width // group_size), kernel_size = (kernel_size, kernel_size), strides = (stride, stride), groups = width // group_size, padding = 'same', name = 'conv1')(results);
  if use_two_convs:
